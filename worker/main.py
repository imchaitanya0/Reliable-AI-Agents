...
---------------------------------
The worker never holds state that is not committed. Every task ends in a single
transaction that marks the row succeeded, merges the result into the agent's
context and advances the cursor. If the process dies at any instruction, the
committed prefix is intact and the lease simply expires -- another worker picks
up at exactly the next task.

The worker also never decides retries or promotions. It moves rows into
'succeeded' or 'failed' and records which of the three failure classes occurred.
Every routing decision belongs to the orchestrator, which is what keeps that
...
def _handle_sigterm(signum: int, frame: object) -> None:
    """Graceful stop: finish the task in hand, then exit. SIGKILL is the demo."""
    global _shutdown
    _shutdown = True
...
# Fenced on lease_owner: if the reaper reclaimed this task while we were slow,
# our lease no longer matches and this affects zero rows. We must then abandon
# the checkpoint rather than commit it -- otherwise we double-advance the cursor.
MARK_SUCCEEDED_SQL = """
...
# Fenced a second time on the cursor itself: we only advance from the exact
# position we were working on, so a replayed checkpoint cannot skip a task.
ADVANCE_AGENT_SQL = """
UPDATE agents
...
def checkpoint_success(
    ...
    """
    Commit a completed task. Returns False if the lease was lost, in which case
    nothing is written.
    """
    with pool().connection() as conn:
        with conn.cursor() as cur:
            ...
            if cur.rowcount == 0:
                conn.rollback()
                log.warning(
                    "lease lost before checkpoint task=%s -- discarding result",
                    task["id"],
                )
                return False
            ...
            if cur.fetchone() is None:
                conn.rollback()
                log.warning(
                    "cursor moved under us agent=%s seq=%s -- discarding",
                    task["agent_id"],
                    task["seq"],
                )
                return False
...
def process_one(
    ...
    try:
        ...
        if agent is None:
            ...
            report_failure(
                task, PoisonFailure("agent row vanished"), started, worker_id
            )
            return True
...
def main() -> int:
    ...
    log.info(
        "worker up tier=%s lease_ttl=%ss tasks_known=%d",
        POOL_TIER,
        LEASE_TTL_SECONDS,
        len(registry),
    )
